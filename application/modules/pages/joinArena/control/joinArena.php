...
$MySQL->executeSQL("SELECT * FROM joinArena WHERE id = $id");

if(isset($_POST['info'])){
	$hero = mysql_real_escape_string($_POST["hero"]);
	$hero2 = mysql_real_escape_string($_POST["hero2"]);
	$captcha = mysql_real_escape_string($_POST["captcha"]);
	$noeevent = mysql_real_escape_string($_POST["noeevent"]);
	...
	if($result == 1) {
		...
		$MySQL->executeSQL("INSERT INTO joinArena (`id`, `hero`, `hero2`, `noeevent`) VALUES (".$id.", '".$hero."', '".$hero2."', '".$noeevent."');");
		$result = 0;
	}
