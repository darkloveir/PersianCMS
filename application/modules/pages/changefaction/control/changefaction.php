...
if(isset($_POST['changerace'])){
    ...
    if($result == 1){
        if($method == "vp" || $method == "dp")
        	$MySQL->executeSQL("UPDATE characters SET at_login = '64' WHERE name = '$character';");
        $MySQL->database = $db_website;
        $MySQL->Connect($persistant);
		if($method == "vp")
    ...
    if($result == 1 && is_numeric($result)) {
        ...
        switch($result){	
            case 0:  $message = '<p class="success"><a href="panel">'.$p_changefaction['25'].'</a></p>'; break;
            case -1: $message = '<p class="error">'.$p_changefaction['26'].'</p>'; break;
            case -2: $message = '<p class="error">'.$p_changefaction['27'].'</p>'; break;
            case -3: $message = '<p class="error">'.$p_changefaction['28'].'</p>'; break;
            case -4: $message = '<p class="error">'.$p_changefaction['29'].'</p>'; break;
			case -5: $message = '<p class="error">'.$p_changefaction['40'].'</p>'; break;
            default: $message = '<p class="error">'.$p_changefaction['30'].'</p>'; break;
        }
    }
}
?>